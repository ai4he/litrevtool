...
export default function CreateJobDialog({ open, onClose, onSubmit }: CreateJobDialogProps) {
  ...
  const handleAddSuggestion = (keyword: string, type: 'include' | 'exclude') => {
    const list = type === 'include' ? formData.keywords_include : formData.keywords_exclude;
    if (!list.includes(keyword)) {
      if (type === 'include') {
        setFormData({ ...formData, keywords_include: [...formData.keywords_include, keyword] });
      } else {
        setFormData({ ...formData, keywords_exclude: [...formData.keywords_exclude, keyword] });
      }
    }
  };
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(e) => setFormData({ ...formData, use_semantic: e.target.checked })}
              />
            }
          ...
          {formData.use_semantic && (
            <Box sx={{ mt: 2 }}>
              ...
              <TextField
                ...
                onChange={(e) => setFormData({ ...formData, semantic_inclusion: e.target.value })}
              ...
              <TextField
                ...
                onChange={(e) => setFormData({ ...formData, semantic_exclusion: e.target.value })}
              ...
              <FormControlLabel
                ...
                label={
                  <Box>
                    <Typography variant="body2">Batch Mode (Recommended)</Typography>
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(e) => setFormData({ ...formData, generate_latex: e.target.checked })}
              />
            }
            label={
              <Box>
                <Typography variant="body2">Generate Research Paper (LaTeX)</Typography>
                <Typography variant="caption" color="textSecondary">
