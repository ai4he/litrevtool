...
interface LastCheckpoint {
  last_year_completed?: string | number;
...
export default function Dashboard() {
  ...
  const [jobPapers, setJobPapers] = useState<Record<number, Paper[]>>({});
  ...
  const handleDownload = async (jobId: number, jobName: string) => {
    try {
  ...
  const handleDownloadPrismaDiagram = async (jobId: number, jobName: string) => {
    try {
  ...
  const handleDownloadLatex = async (jobId: number, jobName: string) => {
    try {
  ...
  const handleDownloadBibtex = async (jobId: number, jobName: string) => {
    try {
  ...
  const fetchPapers = useCallback(async (jobId: number) => {
    try {
  ...
  const fetchScreenshot = useCallback(async (jobId: number) => {
    try {
  ...
  useEffect(() => {
    jobs.forEach(job => {
      if (job.status === 'completed' && job.total_papers_found && job.total_papers_found > 0) {
  ...
  const getStatusColor = (status: Job['status']): 'default' | 'primary' | 'success' | 'error' | 'warning' => {
    const colors: Record<Job['status'], 'default' | 'primary' | 'success' | 'error' | 'warning'> = {
      pending: 'default',
      running: 'primary',
      completed: 'success',
      failed: 'error',
                        ...
                        <Box sx={{ mt: 2, p: 1.5, backgroundColor: 'rgba(25, 118, 210, 0.08)', borderRadius: 1 }}>
                          ...
                          <Typography variant="body2" sx={{ mt: 0.5 }}>
                            {job.last_checkpoint.last_year_completed
                              ? `Year ${parseInt(String(job.last_checkpoint.last_year_completed)) + 1}`
                              : 'Initializing...'}
                          </Typography>
